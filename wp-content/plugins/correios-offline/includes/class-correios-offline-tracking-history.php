 ...
 *
 * @package correios_offline/Classes/Tracking
 * @since   3.0.0
 * @version 3.2.0
 */
 ...
 */
class Correios_Offline_Tracking_History {
	...
	protected function get_tracking_history_webservice_url() {
		return apply_filters( 'correios_offline_tracking_webservice_url', $this->_webservice_url );
	}
	...
	protected function get_user_data() {
		$user_data = apply_filters( 'correios_offline_tracking_user_data', array(
			'login'    => 'ECT',
			'password' => 'SRO',
		) );
	...
	protected function logger( $data ) {
		if ( apply_filters( 'correios_offline_enable_tracking_debug', false ) ) {
	...
	protected function get_tracking_history( $tracking_codes ) {
		...
		$args      = apply_filters( 'correios_offline_tracking_history_webservice_args', array(
		...
		try {
			$soap     = new Correios_Offline_Soap_Client( $this->get_tracking_history_webservice_url() );
		...
		return apply_filters( 'correios_offline_tracking_objects', $objects, $tracking_codes );
	}
	...
	public function view( $order ) {
		...
		if ( apply_filters( 'correios_offline_enable_tracking_history', false ) ) {
		...
		wc_get_template(
			...
			array(),
			'',
			Correios_Offline::get_templates_path()
		);
...
new Correios_Offline_Tracking_History();
