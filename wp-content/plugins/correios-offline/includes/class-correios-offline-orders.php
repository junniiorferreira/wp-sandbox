 ...
 *
 * @package correios_offline/Classes/Orders
 * @since   3.7.0
 * @version 3.7.0
 */
...
class Correios_Offline_Orders {
	...
	public function shipping_method_delivery_forecast( $name, $order ) {
		...
		foreach ( $order->get_shipping_methods() as $shipping_method ) {
			...
			if ( $total ) {
				...
				$names[] = sprintf( _n( '%1$s (delivery within %2$d working day after paid)', '%1$s (delivery within %2$d working days after paid)', $total, 'correios-offline' ), $shipping_method->get_name(), $total );
			} else {
	...
	public function item_display_delivery_forecast( $display_key, $meta ) {
		return '_delivery_forecast' === $meta->key ? __( 'Delivery forecast', 'correios-offline' ) : $display_key;
	}
}

new Correios_Offline_Orders();
