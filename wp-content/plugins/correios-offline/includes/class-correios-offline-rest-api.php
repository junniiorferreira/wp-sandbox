 ...
 */
class Correios_Offline_REST_API {
	...
	public function legacy_orders_response( $data, $order, $fields ) {
		$data['correios_tracking_code'] = implode( ',', Correios_Offline_get_tracking_codes( $order ) );
	...
	public function legacy_orders_update( $order_id, $data ) {
		if ( isset( $data['correios_tracking_code'] ) ) {
			Correios_Offline_update_tracking_code( $order_id, $data['correios_tracking_code'] );
		}
	}
	...
	public function register_tracking_code() {
		...
		register_rest_field( 'shop_order',
			...
			array(
				...
				'schema'          => array(
					'description' => __( 'Correios tracking code.', 'correios-offline' ),
					'type'        => 'string',
	...
	public function get_tracking_code_callback( $data, $field, $request ) {
		return implode( ',', Correios_Offline_get_tracking_codes( $data['id'] ) );
	}
	...
	public function update_tracking_code_callback( $value, $object ) {
		...
		return Correios_Offline_update_tracking_code( $object->ID, $value );
	}
}

new Correios_Offline_REST_API();
