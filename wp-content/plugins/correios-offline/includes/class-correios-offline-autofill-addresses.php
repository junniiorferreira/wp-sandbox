 ...
 *
 * @package correios_offline/Classes/Autofill
 * @since   3.0.0
 * @version 3.0.0
 */
 ...
 */
class Correios_Offline_Autofill_Addresses {
	...
	public function init() {
		if ( apply_filters( 'correios_offline_enable_autofill_addresses', false ) ) {
			$this->maybe_install();
	...
	protected static function get_tracking_addresses_webservice_url() {
		return apply_filters( 'correios_offline_addresses_webservice_url', self::$_webservice_url );
	}
	...
	protected static function logger( $data ) {
		if ( apply_filters( 'correios_offline_enable_autofill_addresses_debug', false ) ) {
	...
	protected static function get_validity() {
		return apply_filters( 'correios_offline_autofill_addresses_validity', 'forever' );
	}
	...
	public static function get_address( $postcode ) {
		...
		$postcode = Correios_Offline_sanitize_postcode( $postcode );
	...
	protected static function fetch_address( $postcode ) {
		include_once dirname( __FILE__ ) . '/class-correios-offline-soap-client.php';
		...
		try {
			$soap     = new Correios_Offline_Soap_Client( self::get_tracking_addresses_webservice_url() );
	...
	public function frontend_scripts() {
		if ( is_checkout() || is_account_page() ) {
			...
			wp_enqueue_script( 'correios-offline-autofill-addresses', plugins_url( 'assets/js/frontend/autofill-address' . $suffix . '.js', Correios_Offline::get_main_file() ), array( 'jquery', 'jquery-blockui' ), Correios_Offline_VERSION, true );

			wp_localize_script(
				'correios-offline-autofill-addresses',
				'WCCorreiosAutofillAddressParams',
				array(
					'url'   => WC_AJAX::get_endpoint( $this->ajax_endpoint ),
					'force' => apply_filters( 'correios_offline_autofill_addresses_force_autofill', 'no' ),
				)
			);
		}
	}
	...
	public function ajax_autofill() {
		if ( empty( $_GET['postcode'] ) ) { // WPCS: input var okay, CSRF ok.
			wp_send_json_error( array( 'message' => __( 'Missing postcode paramater.', 'correios-offline' ) ) );
			exit;
		}

		$postcode = Correios_Offline_sanitize_postcode( wp_unslash( $_GET['postcode'] ) ); // WPCS: input var okay, CSRF ok.

		if ( empty( $postcode ) || 8 !== strlen( $postcode ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid postcode.', 'correios-offline' ) ) );
			exit;
		}
	...
	public function maybe_install() {
		$version = get_option( 'correios_offline_autofill_addresses_db_version' );

		if ( empty( $version ) ) {
			self::create_database();

			update_option( 'correios_offline_autofill_addresses_db_version', '1.0.0' );
		}
	}
...
new Correios_Offline_Autofill_Addresses();
