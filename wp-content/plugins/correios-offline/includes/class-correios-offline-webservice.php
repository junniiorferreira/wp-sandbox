 ...
 * Correios Webservice.
 *
 * @package correios_offline/Classes/Webservice
 * @since   3.0.0
 * @version 3.0.0
 */
 ...
 */
class Correios_Offline_Webservice {
	...
	public function set_package( $package = array() ) {
		...
		$correios_package = new Correios_Offline_Package( $package );
	...
	public function get_webservice_url() {
		return apply_filters( 'correios_offline_webservice_url', $this->_webservice, $this->id, $this->instance_id, $this->package );
	}
	...
	public function get_origin_postcode() {
		return apply_filters( 'correios_offline_origin_postcode', $this->origin_postcode, $this->id, $this->instance_id, $this->package );
	}
	...
	public function get_login() {
		return apply_filters( 'correios_offline_login', $this->login, $this->id, $this->instance_id, $this->package );
	}
	/**
	 * Get password.
	 *
	...
	public function get_password() {
		return apply_filters( 'correios_offline_password', $this->password, $this->id, $this->instance_id, $this->package );
	}
	...
	public function get_shipping() {
		...
		$args = apply_filters( 'correios_offline_shipping_args', array(
			'nCdServico'          => $this->service,
			'nCdEmpresa'          => $this->get_login(),
			'sDsSenha'            => $this->get_password(),
			'sCepDestino'         => Correios_Offline_sanitize_postcode( $this->destination_postcode ),
			'sCepOrigem'          => Correios_Offline_sanitize_postcode( $this->get_origin_postcode() ),
		...
		if ( is_wp_error( $response ) ) {
			...
			try {
				$result = Correios_Offline_safe_load_xml( $response['body'], LIBXML_NOCDATA );
			} catch ( Exception $e ) {
				if ( 'yes' === $this->debug ) {
